(function () {
  ...
  function renderSummary(data) {
    ...
    return (
      '<div class="row g-4">' +
      section("Identificação da Ocorrência", "Contexto principal, local e classificação do registro.", field("Data da Inspeção", data.data_hora_inicio) + field("Data/Hora de Encerramento", data.data_hora_fim) + '<div class="col-md-4"><label class="form-label text-muted">Status</label><div><span class="badge badge-' + escapeHtml(data.status_badge) + '">' + escapeHtml(data.status_label) + "</span></div></div>" + field("Responsável pela Inspeção", data.responsavel_registro, "col-md-6") + field("Responsável Técnico", data.responsavel_tecnico) + field("Área", data.area) + field("Localização", data.local) + field("Tipo de Himenóptero", data.tipo_himenoptero) + field("Nome Popular", data.popular) + field("Espécie", data.especie) + field("Proximidade de pessoas", data.proximidade_pessoas) + field("Classificação do Risco", data.classificacao_risco) + field("Condição", data.condicao) + field("Ação Realizada", data.acao_realizada) + field("Isolamento de Área", data.isolamento_area)) +
